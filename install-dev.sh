...
install_slack_cli() {
        ...
        echo "Extracting the Slack CLI (build: ${LATEST_SLACK_CLI_DEV_VERSION}): $slack_cli_bin_path"
        ...
        if [ $(command -v $slack_cli_name) ]; then
                echo -e "\n✨ Slack CLI (build: ${LATEST_SLACK_CLI_DEV_VERSION}) was installed successfully\n"
        else
                echo "Manually add the Slack CLI (build: ${LATEST_SLACK_CLI_DEV_VERSION}) directory to your \$HOME/$shell_profile (or similar)"
